const func = () => {
    ...
    let unit = parseFloat(u1)+parseFloat(u2)+parseFloat(u3)+parseFloat(u4)+parseFloat(u5)+parseFloat(u6)+parseFloat(u7)+parseFloat(u8)+parseFloat(u9)+parseFloat(u10)+parseFloat(u11);

    let total = (avg1 + avg2 + avg3 + avg4 + avg5 + avg6 + avg7 + avg8 + avg9 + avg10 + avg11)/unit;

    let gwa = total.toPrecision(5);

    if(gwa<=1.45 && g1<=2.4 && g2<=2.4 && g3<=2.4 && g4<=2.4 && g5<=2.4 && g6<=2.4 && g7<=2.4 && g8<=2.4 && g9<=2.4 && g10<=2.4 && g11<=2.4 && u1>0 && u2>0 && u3>0 && u4>0 && u5>0 && u6>0 && u7>0 && u8>0 && u9>0 && u10>0 && u11>0){
        document.getElementById('showData-2').innerHTML = name;
        document.getElementById('showData-3').innerHTML = gwa;
        document.getElementById('showData-1').innerHTML = `<strong style="color:yellow;"> PRESIDENT'S LISTER </strong>`
    }
    else if(gwa<=1.75 && g1<=2.4 && g2<=2.4 && g3<=2.4 && g4<=2.4 && g5<=2.4 && g6<=2.4 && g7<=2.4 && g8<=2.4 && g9<=2.4 && g10<=2.4 && g11<=2.4 && u1>0 && u2>0 && u3>0 && u4>0 && u5>0 && u6>0 && u7>0 && u8>0 && u9>0 && u10>0 && u11>0){
        document.getElementById('showData-2').innerHTML = name;
        document.getElementById('showData-3').innerHTML = gwa;
        document.getElementById('showData-1').innerHTML = `<strong style="color:orange;"> DEAN'S LISTER </strong>`
    } else if (gwa<=5 && g1>0 && g1<=5 && g2>0 && g2<=5 && g3>0 && g3<=5 && g4>0 && g4<=5 && g5>0 && g5<=5 && g6>0 && g6<=5 && g7>0 && g7<=5 && g8>0 && g8<=5 && g9>0 && g9<=5 && g10>0 && g10<=5 && g11>0 && g11<=5 && u1>0 && u2>0 && u3>0 && u4>0 && u5>0 && u6>0 && u7>0 && u8>0 && u9>0 && u10>0 && u11>0){
        document.getElementById('showData-2').innerHTML = name;
        document.getElementById('showData-3').innerHTML = gwa;
        document.getElementById('showData-1').innerHTML = `<h3 style="color:white; font-size:15px">Just a little more, you’ve got this!</h3>`
    } else {
